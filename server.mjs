...
async function wp(path, params = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Learn One Tech API returned ${response.status}: ${body.slice(0, 300)}`);
  }
...
server.tool(
  "site_info",
  "Get basic information about Learn One Tech and its public WordPress API.",
  {},
  async () => ({
    content: [{
      type: "text",
      text: JSON.stringify({
        ...
        api: `${API_URL}/`,
        note: "Content is fetched from the public Learn One Tech website. Use device-recovery guidance only for devices you own or are authorized to service.",
      }, null, 2),
    }],
  })
);
...
server.tool(
  "search_articles",
  "Search Learn One Tech articles by keyword. Returns titles, excerpts, dates and URLs.",
  {
  ...
  async ({ query, limit }) => {
    ...
    return {
      content: [{ type: "text", text: JSON.stringify({ query, results: posts.map(summarizePost) }, null, 2) }],
    };
  }
);
...
server.tool(
  "list_articles",
  "List recent Learn One Tech articles with pagination.",
  {
  ...
  async ({ page, per_page, category_id }) => {
    ...
    return {
      content: [{ type: "text", text: JSON.stringify({ page, per_page, results: posts.map(summarizePost) }, null, 2) }],
    };
  }
);
...
server.tool(
  "get_article",
  "Retrieve a Learn One Tech article by numeric WordPress post ID or exact slug. Returns cleaned readable text plus the canonical URL.",
  {
...
server.tool(
  "list_categories",
  "List Learn One Tech WordPress article categories.",
  {
...
server.tool(
  "search_site",
  "Search Learn One Tech posts and pages using WordPress's public search endpoint.",
  {
